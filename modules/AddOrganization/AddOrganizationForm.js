...
export default function AddOrganizationForm({ session }) {
  ...
  const handleSubmit = async (e) => {
    var data = JSON.stringify({
      query: `mutation{
        createOrganization(name:"${name}", userId:"${session.user.id}"){
            name,
             id
        }
    }`,
      ...
      <div className="space-y-8 divide-y divide-gray-200">
        <div>
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Use this form to describe your new organization.</p>
          </div>
